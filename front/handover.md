...
| フェーズ                          | ブランチ         | 状態                       |
| --------------------------------- | ---------------- | -------------------------- |
| Phase 1 (PWA scaffold)            | `feat/pwa-setup` | ✅ `frontend` にマージ済み |
| Phase 2 (認証)                    | `feat/login`     | ✅ 実装済み（未マージ）    |
| Phase 3 (ルーム一覧・sync)        | `feat/login`     | ✅ 実装済み（未マージ）    |
| Phase 4 (タイムライン + 過去ログ) | `feat/timeline`  | ⬜ 未実装                  |
| Phase 5 (モバイル UX)             | `feat/mobile-ux` | ⬜ 未実装                  |

## 次のアクション

```sh
# feat/login を frontend にマージしてリリース
git checkout frontend
git merge feat/login --no-ff -m "feat(front): merge feat/login — Phase 2 + 3 (auth, room list, sync, timeline)"
git push origin frontend

# Phase 4 (タイムライン + 過去ログ) を開始する
git checkout -b feat/timeline
```

### Phase 4 実装内容

- `GET /rooms/{roomId}/messages` を使った過去ログ遡り（上スクロールでページング）
- `stores/rooms.ts` に `loadHistory(roomId)` アクション追加
- `RoomPage.tsx` で IntersectionObserver によるペーパーロード
- 既読送信: `POST /rooms/{roomId}/receipt/m.read/{eventId}`
